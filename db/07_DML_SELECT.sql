...
속성명1 ad 속성명2 => 속성명1을 일시적으로 속성명2로 사용
...
select distince 속성명 from 테이블명;
*/
/*
- ANY
where 속성명 비교연산자(=, >=, <=, !=, <, >) any(서브쿼리);
=> 속성이 서브쿼리 결과와 비교연산을 했을 때 서브쿼리 경과 중 하나이상 만족하면 조회
where 속성명 = any(서브쿼리)
=> where 속성명 in(서브쿼리)
...
select subject_code as 과목코드, count(*) as '수강생 수' from course group by subject_code;
-- 과목별 수강생 수가 4명 이상인 과목의 과목코드를 조회하는 쿼리
select 
	subject_code as 과목코드, count(*) as '수강생 수' 
from 
	course 
group by 
	subject_code
